...
model.load_state_dict(torch.load('ddpm_model_fashion.pt'))
model.to(device)
...
def get_mse(images):
    ...
    noise_img = images
    pre_img, x_start = loop(diffusion, noise_img)
    images = images.repeat(1, 3, 1, 1)
    x_start = x_start.repeat(1, 3, 1, 1)
    images1 = images.clone()
    start = x_start.clone()
    images1[images1 <= 0] = 1  # 将黑色背景部分转化为白色
    start[start <= 0] = 1
    j = 0
    while j <= 63:
        loss_function = torch.nn.MSELoss()
        mse_value = loss_function(start[j], images1[j])
        mse_value = torch.sqrt(mse_value)
...
class MLPBinaryClassifier(nn.Module):
    def __init__(self):
        ...
        self.mse = get_mse
        self.fc1 = nn.Linear(1, 1024)  # 输入层到隐藏层
...
#
#         outputs = model(data)  # 将一维数据扩展为二维张量1
#
...
# torch.save(model.state_dict(),f'fashionMLP_2000.pt')
model.load_state_dict(torch.load(f'fashionMLP_2000.pt'))
# 在测试集上评估模型
# 测试模型
